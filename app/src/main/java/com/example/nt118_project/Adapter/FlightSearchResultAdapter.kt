...
class FlightSearchResultAdapter(private var dataList: ArrayList<FlightTicket>, private var context: Context): RecyclerView.Adapter<FlightSearchResultAdapter.FlightSearchResultViewHolder>() {

    public var onItemClick: ((FlightTicket) -> Unit)? = null
    inner class FlightSearchResultViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): FlightSearchResultViewHolder {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.detail_list_of_flights_search_result,parent,false)
        return FlightSearchResultViewHolder(itemView)
    }
    ...
    override fun onBindViewHolder(holder: FlightSearchResultViewHolder, position: Int) {
        ...
        holder.tvDate.setText(currItem.Date)
